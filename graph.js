...
const graph = svg
  ...
  .attr(
    "transform",
    `translate(${dims.width / 2 + 5}, ${dims.height / 2 + 5})`
  );
...
// légendes
const groupeLegendes = svg
  .append("g")
  .attr("transform", `translate(${dims.width + 40}, 10)`);

const legendes = d3.legendColor().shape("circle").scale(couleur);
...
const maj = (donnee) => {
  ...
  groupeLegendes.selectAll("text").attr("fill", "#fff");

  // liaison path->données
  const paths = graph.selectAll("path").data(pie(donnee));
